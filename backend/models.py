...
class Article(Base):
    ...
    origin_url = Column(Text, comment="原始链接(方便回溯)")
    ...
    # 解读归属：若为解读则属于某篇新闻；新闻则为空
    news_id = Column(Integer, ForeignKey("articles.id"), nullable=True, index=True, comment="所属新闻ID(仅解读类型有值)")

    # 关联关系
    category = relationship("Category", back_populates="articles")
    favorited_by = relationship("Favorite", back_populates="article")
    news = relationship("Article", remote_side=[id], backref="interpretations", foreign_keys=[news_id])
