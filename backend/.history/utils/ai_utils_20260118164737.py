...
async def web_search(query: str, max_results: int = 5, days: int = 7) -> Dict:
    """
...
async def generate_article_from_search(query: str):
    ...
    search_response = await web_search(query, max_results=5, days=7) 
    ...
    # 5. 构建 Prompt
    system_prompt = f"""你是一名专业的政策原文整理员，专注于中国政策内容的准确还原。今天是 {today_date}。
请根据搜索结果生成一篇政策原文整理文章，要求：

1. **严格按照政策原文内容整理，不得添加任何解读、分析或评论**
2. 重点还原中国的农学政策或其他相关政策的具体内容
3. 保持政策原文的权威性、准确性和完整性，不得随意篡改或歪曲
4. 优先引用官方来源的政策内容
5. 注明政策的发布单位、时间和来源（如果搜索结果中包含）
6. 保留政策原文的核心要点和关键信息
7. 不要添加任何个人观点或解读
