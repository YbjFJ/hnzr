...
from models import Article, ChatSession, ChatMessage, Favorite, User, ArticleStatus
...
from constant.user_constant import system_prompt,KEY_WORD
...
async def stream_chat_message(
    ...
    async def generate_response():
        ...
        try:
            ...
            # ----- 用户指定了收藏文章作为知识库 -----
            use_favorite_knowledge = message.ref_article_ids and message.ref_article_ids.strip()
            favorite_articles = []
            if use_favorite_knowledge:
                ids_str = [x.strip() for x in message.ref_article_ids.split(",") if x.strip()]
                ids_int = []
                for s in ids_str:
                    try:
                ...
                if ids_int:
                    ...
                    favorite_articles = db.query(Article).join(Favorite).filter(
                        ...
                        Article.id.in_(ids_int),
                        Article.status == ArticleStatus.PUBLISHED
                    ).all()
                    use_favorite_knowledge = len(favorite_articles) > 0

            if use_favorite_knowledge and favorite_articles:
                yield f"> 📚 **系统状态**: 正在使用您选择的 {len(favorite_articles)} 篇收藏文章作为知识库...\n\n"
                ref_article_ids_list = [str(a.id) for a in favorite_articles]
                k = min(10, max(3, len(favorite_articles) * 2))
                retrieved_docs = retrieve_relevant_content(message.content, favorite_articles, k=k)
                if retrieved_docs:
                    context += "根据以下您选择的收藏文章内容，为用户提供回答：\n\n"
                    for doc in retrieved_docs:
                        context += f"【参考资料】:\n{doc.page_content[:300]}...\n\n"
                    yield "> ✅ **系统状态**: 资料分析完成，正在生成回答...\n\n---\n\n"
                else:
                    # 向量检索无结果时仍用文章摘要等作为上下文
                    context += "根据以下您选择的收藏文章摘要，为用户提供回答：\n\n"
                    for a in favorite_articles:
                        context += f"【{a.title}】:\n{(a.summary or a.content[:200])}...\n\n"
                    yield "> ✅ **系统状态**: 正在生成回答...\n\n---\n\n"
            else:
                # --- 阶段 1: 关键词提取 ---
                yield "> 🔍 **系统状态**: 正在分析您的问题，提取并扩展核心搜索词...\n\n"
                keyword_prompt = ChatPromptTemplate.from_template(KEY_WORD)
                keyword_chain = keyword_prompt | llm | StrOutputParser()
                extracted_keywords_str = await keyword_chain.ainvoke({"question": message.content})
                keywords = list(set([k.strip() for k in extracted_keywords_str.split() if k.strip()]))
                keywords_display = " ".join(keywords)
                yield f"> 🗝️ **系统状态**: 已提取关键词：`{keywords_display}`\n\n"
                await asyncio.sleep(0.1)

                # --- 阶段 2: 基于关键词的数据库检索 ---
                yield "> 📚 **系统状态**: 正在根据关键词检索知识库...\n\n"
                if keywords:
                ...
                if search_articles:
                    ...
                    if retrieved_docs:
                        ...
                        yield "> ⚠️ **系统状态**: 未找到高相关性段落，将基于通用知识回答...\n\n---\n\n"
                else:
                    yield f"> 🌐 **系统状态**: 知识库未找到相关内容，正在尝试联网搜索...\n\n"
            ...
            final_system_prompt = system_prompt1.replace("{context}", formatted_context)
